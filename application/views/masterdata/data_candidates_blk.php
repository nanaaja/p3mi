...
<section class="content">
        ...
        <div class='col-md-12'>
            <div class='box '>
                <div class='box-header'>
                    <h3 class='box-title'>Data BLK-LN </h3>
                    <button class="btn btn-primary my-2 pull-right mx-3" data-toggle="modal" data-target="#add-blk"><i class="fa fa-plus"></i> Add Data</button> &nbsp;
                ...
                <div class='box-body pad'>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered" id="blk-table" width="100%" cellspacing="0">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>No</th>
    ...
    <!-- Modal tambah data -->
    <div class="modal fade add-blk" id="add-blk" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Add Hasil BLK-LN</h5>
                ...
                <div class="modal-body">
                    <form id="tambah-blk" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="exampleFormControlSelect1">BLK</label>
                            <select class="form-control" style="width:100%" name="blk_id" id="blk_id">
                                <option value=''>Choose BLK</option>
                                <?php foreach ($data_blk as $r) { ?>
                                    <option value='<?= $r->blk_id ; ?>'><?= $r->blk_name; ?></option>
                                <?php } ?>
                            </select>
                            <input type="hidden" class="form-control" name="tipe_form" value="add">
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="date_start" name="date_start" placeholder="12/01/1994" required>
                            <input type="hidden" class="form-control" name="tipe_form" value="add">
                        </div>
                        <div class="form-group">
                            <label for="exampleFormControlInput1">Date Finish</label>
                            <input type="date" class="form-control" id="date_finish" name="date_finish" placeholder="12/01/1994" required>
                        </div>
                        <div class="form-group">
                            <label for="exampleFormControlSelect1">Status</label>
                            <select class="form-control" id="cb_status" name="cb_status" required>
    ...
    <div class="modal fade" id="m_import" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var table = $('#blk-table').DataTable({
                    "ajax": {
                        url: '<?php echo site_url("MasterData/candidates_blk_page"); ?>',
                        type: 'POST'
                    },
                    ...
                    "columnDefs": [{
...

                        {
                            ...
                            "render": function(data, type, row, meta) {
								if(role == 'superadmin'){
									return "<a href=<?php echo site_url('MasterData/edit_candidates_blk/'); ?>" +
										data +
										"><button class='btn btn-primary btn-md' name='edit'><i class='fa fa-edit  text-white-50 mr-1'></i>  </button></a> <button type='button' class='btn btn-danger btn-md' name='hapus' blk_id=" +
                ...
                $('#tambah-blk').on('submit', function(event) {
                    event.preventDefault();
                    $.ajax({
                        type: "POST",
                        url: '<?php echo site_url('MasterData/add_candidates_blk'); ?>',
                        ...
                        success: function(data) {
                            ...
                            Toast.fire({
                                type: 'success',
                                title: 'blk Berhasil ditambahkan'
                            });
                            $('#tambah-blk').trigger("reset");
                            setTimeout(function() {
                                window.location.href =
                                    '<?php echo site_url('MasterData/candidates_blk'); ?>';
                                window.clearTimeout();
                            }, 1000);
                ...
                $(document).on('click', '.add-blk', function() {
                    var id_po_produk = $(this).attr('id_po_produk');
                    $('#m_add_blk').modal('show');
                });
            ...
            function edit(id) {
                ...
                console.log(blk_id)
                Swal.fire({
                    title: 'Are you sure delete blk this..?',
                    text: "",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: '<?php echo site_url('MasterData/hapus_candidates_blk'); ?>',
                            data: {
                                blk_id: blk_id
                            },
                            dataType: "json",
