        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <form action="<?= site_url('ImportController/import_excel_saksi'); ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <a href="<?php echo base_url('uploads/template_saksi_new_update.xlsx'); ?>">Download Template Import </a><br><br>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                var table = $('#tps-table').DataTable({
                    "ajax": {
                        url: '<?php echo site_url("MasterData/saksi_page"); ?>',
                        type: 'POST'
                    },
                    ...
                    "columnDefs": [{
                            ...
                            "render": function(data, type, row, meta) {
                                 //if(role == 'superadmin'){
                                    return "<a target=_blank href=<?php echo site_url('MasterData/print_saksi/'); ?>" +
                                    data +
                                    "><button class='btn btn-primary btn-md' name='edit'><i class='fa fa-print  text-white-50 mr-1'></i>  </button>";
                                 //}else{
                                 //    return null;
                                 //}
                            }
			...
			function print(id) {
                ...
                var id_saksi = id;
				alert(id_saksi);
                console.log(id_saksi)
                Swal.fire({
                    title: 'Anda yakin print surat..?',
                    text: "",
                    icon: 'warning',
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: '<?php echo base_url('MasterData/generate_pdf'); ?>',
                            data: {
                                id_saksi: id_saksi
                            },
                            dataType: "json",
                            success: function(data) {
                                ...
                                setTimeout(function() {
                                    window.location.href =
                                        '<?php echo site_url('MasterData/saksi'); ?>';
                                    window.clearTimeout();
                                }, 1000);
